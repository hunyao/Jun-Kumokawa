...
import { type ProjectTab, ProjectTabs } from '../constants';

type ProjectProps = {
  experience: Experience;
  project: Project;
};
export const ProjectComponent = (props: ProjectProps) => {
  const { experience, project } = props;
  const { i18n, t } = useLingui();
  const { lang } = useContext(TranslateContext);
  const [tab, setTab] = useState<ProjectTab>(ProjectTabs.RESPONSEBILITY);
  const id = useId();
          ...
          <div className='collapse overflow-visible'>
            ...
            <div className='collapse-content opacity-0 transition-all duration-300 peer-checked:opacity-100'>
              ...
              <GithubTab $variant='border' className='mt-4'>
                <GithubTabItem
                  $active={tab === ProjectTabs.RESPONSEBILITY}
                  onClick={() => setTab(ProjectTabs.RESPONSEBILITY)}
                >
                  <Trans>Responsibilities</Trans>
                </GithubTabItem>
                <GithubTabItem
                  $active={tab === ProjectTabs.ACHIEVEMENT}
                  onClick={() => setTab(ProjectTabs.ACHIEVEMENT)}
                >
                  <Trans>Achievements</Trans>
                </GithubTabItem>
                <GithubTabItem
                  $active={tab === ProjectTabs.TECH}
                  onClick={() => setTab(ProjectTabs.TECH)}
                >
                  <Trans>Technologies</Trans>
                </GithubTabItem>
                <GithubTabItem
                  $active={tab === ProjectTabs.SCOPE}
                  onClick={() => setTab(ProjectTabs.SCOPE)}
                >
                  <Trans>Scopes</Trans>
                </GithubTabItem>
              </GithubTab>
              <div className='p-4 pb-0'>
                <ul
                  ...
                  hidden={tab !== ProjectTabs.RESPONSEBILITY}
                >
                ...
                <ul
                  className='list list-inside list-disc'
                  hidden={tab !== ProjectTabs.ACHIEVEMENT}
                >
                ...
                <div hidden={tab !== ProjectTabs.SCOPE}>
                  <div>
